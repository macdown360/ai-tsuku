...
このプラットフォームは、検索エンジン最適化（SEO）を重視した実装をしています：

### 🔍 実装済み SEO 対策

1. **メタデータの動的生成**
   - ページごとに最適化されたタイトル・ディスクリプション
   - キーワード自動抽出（カテゴリ・タグ・ツール情報から）
   - Canonical URL 設定で重複ページ対策

2. **構造化データ（Schema.org）**
   - **Article スキーマ**: プロジェクト詳細ページ
   - **BreadcrumbList スキーマ**: サイト階層構造を検索エンジンに明示
   - Google リッチスニペット対応

3. **Sitemap 自動生成**（`/sitemap.xml`）
   - 全プロジェクトページを自動リスト化
   - 更新頻度と優先度を設定
   - 検索エンジンの巡回を効率化

4. **robots.txt 最適化**（`/public/robots.txt`）
   - クローリング対象・除外ページを明確に指定
   - クローラーの低速化対策
...
SEO に関する詳細な設定・運用ガイドは [**SEO_GUIDE.md**](SEO_GUIDE.md) を参照してください。
